...
RreqHeader::RreqHeader (uint8_t flags, uint8_t reserved, uint8_t hopCount, uint32_t requestID, Ipv4Address dst,
                        uint32_t dstSeqNo, Ipv4Address origin, uint32_t originSeqNo, uint8_t etxMetric)
  : m_flags (flags),
    m_reserved (reserved),
    m_hopCount (hopCount),
...
{
  return (23); // ETX field is embedded in reserved flag bits to reduce overhead
}
...
//  i.WriteU8 (m_reserved);
  i.WriteU8 (m_etxMetric); // ETX field is embedded in reserved flag bits
...
RreqHeader::Deserialize (Buffer::Iterator start)
{
  ...
  m_flags = i.ReadU8 ();
//  m_reserved = i.ReadU8 ();
  m_etxMetric = i.ReadU8 (); // ETX field is embedded in reserved flag bits
  m_hopCount = i.ReadU8 ();
...
RreqHeader::Print (std::ostream &os) const
{
  os << "RREQ ID " << m_requestID << " destination: ipv4 " << m_dst
     ...
     << " ETX metric " << (uint32_t)m_etxMetric
...
RrepHeader::RrepHeader (uint8_t prefixSize, uint8_t hopCount, Ipv4Address dst,
                        uint32_t dstSeqNo, Ipv4Address origin, Time lifeTime, uint8_t etxMetric)
  : m_flags (0),
...
{
  return 19+1; // ETX field is 1B added at the end of RREP
}
...
RerrHeader::SetNoDelete (bool f )
{
  if (f)
    {
      m_flag |= (1 << 7); // BUG: bit 7, not bit 0 like it was in original implementation
    }
  else
    {
      m_flag &= ~(1 << 7); // BUG: bit 7, not bit 0 like it was in original implementation
...
{
  return (m_flag & (1 << 7)); // BUG: bit 7, not bit 0 like it was in original implementation
}
...
{
  //return (9);
  return 5; // deleted originator IP 
}
...
{
  os << "Lpp ID: " << m_lppId;
//  os << "Originator IP address: " << m_originAddr;
...
{
//  return (m_lppId == o.m_lppId && m_originAddr == o.m_originAddr && m_originSeqno == o.m_originSeqno);
  return (m_lppId == o.m_lppId && m_originSeqno == o.m_originSeqno);
}
